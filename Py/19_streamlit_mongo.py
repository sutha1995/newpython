...
def delete_post(post_id):
    ...
    try:
        reponse = requests.delete(f"{API_BASE_URL}/posts/{post_id}")
        return response.json(), response.status_code == 200
    except Exception as e:
        return {"error": str(e)}, False
...
def users_page():
    ...
    with tab3:
        ...
        if success and users:
            ...
            selected_user_display = st.selectionbox("Select a user to manage", list(user_options.keys()))
            ...
            if selected_user_display:
                ...
                with col2:
                    ...
                    st.warning("This will delet the user and their posts!")
    ...
    with tab1:
        ...
        if users_success and users:
            with st.form("create_post_form"):
                #User selection
                user_options = {f"{user['name']} ({user['email']})": user['id'] for user in users}
                selected_user_display = st.selectionbox("Select User", list(user_options.keys()))
                ...
                if submitted:
                    if selected_user_display and title and content:
                        ...
                        st.error(f"X Please fill in all fields")
        else:
            st.warning("No users found. Pease create a user first.")
    ...
    with tab3:
        ...
        if users_success and users:
            ...
            if selected_user_display:
                ...
                if success and posts:
                    st.writes(f"**Posts by {selected_user_display}:**")
                    for post in posts:
    ...
    if users_success and posts_success:\
        ...
        if posts:
            ...
            for post in recent_posts:
                st.write(f". **{post['title']}** - {pd.to_datetime(posts['created_at']).strftime('%Y-%m-%d %H:%M')}")
